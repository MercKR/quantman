...
def _write_updater_bat(bat_path: Path, src_dir: Path, dst_dir: Path,
    ...
    v0.9.8-beta 강화 — v0.9.7-beta까지의 결함 fix:
      - 옛: `taskkill /F /PID <parent_pid>` 단일 PID만 죽임 → "이미 실행 중"
        모달 dialog 등 *다른 PID*의 좀비 인스턴스가 살아남아 .dll 매핑 유지 →
        robocopy /R:30 무한 retry → :FAIL 도달 못 한 채 사용자 강제 종료
        → 임시 폴더 누적 + 사용자에게 "그냥 종료된 줄" 인식.
      - 새: `taskkill /F /IM <name> /T` 이미지 이름 + tree 모든 인스턴스 일소.
        retry는 /R:5로 단축 — /IM /T 후 잠금이 풀려있을 게 정상, 안 풀리면 진짜
        시스템 문제이므로 사용자에게 즉시 알림이 옳음.
      - :FAIL 분기: MessageBox 먼저, rmdir 나중. 사용자가 src 폴더 위치 확인 후
        수동 설치 가능하게.

    동작:
      1. taskkill /F /IM /T — 본체 + 모든 인스턴스(좀비 포함) tree 일소.
      2. 잠금 해제 폴링 — 보통 즉시. 최대 10초 대기.
      3. robocopy /E /XO /R:5 /W:1 — 10초/파일 retry. /IM /T 후 잠금 풀려있을 것.
    ...
    content = (
        "@echo off\r\n"
        "REM Quantman auto-updater v0.9.8 (auto-generated by app).\r\n"
        "REM image-name + tree kill로 좀비 인스턴스까지 일소.\r\n"
